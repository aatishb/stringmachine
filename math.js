...
function pointOfIntersection(line1, line2) {
  ...
  if (abs(denominator) > cutoff) {
    ...
    if (abs(numerator1) < cutoff || abs(numerator2) < cutoff) { // lines are overlapping
      if(debugMode){console.log("coincident lines detected");}
      if (lineContainsPoint(line1, line2.start)) { // lines overlap
...
// line object
function makeNewLine(start, end) {
...
/*
function subdivideLine(myLine) {
  subdivisionCount++;
  for (let myIntersection of intersections) {
    let myPoint = myIntersection.point;
    if (myPoint.dist(myLine.start) > 0 && myPoint.dist(myLine.end) > 0) {
      // check if they are strictly within the line segment
      if (lineContainsPoint(myLine, myPoint)) {
        deleteElement(lines, myLine);
        let a = new makeNewLine(myLine.start, myPoint);
        let b = new makeNewLine(myPoint, myLine.end);
        if (!containsLine(lines, a)) {
          lines.push(a);
        }
...
function containsLine(array, myLine) {
  for (let iterLine of array) {
    if (hasSameCoordinates(myLine, iterLine)) {
      return true;
    }
  }
...
function pruneLines(){
  for (let myLine of lines.slice())
  {
    ...
    if(countIntersections < 2 && myLine.lineLength() < 0.9*spacing){
